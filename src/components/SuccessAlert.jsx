...
const SuccessAlert = ({ encabezado, message, visible, onClose }) => {
  const [show, setShow] = useState(true);

  useEffect(() => {
    if (visible) {
      setShow(true);
      setTimeout(() => setShow(false), 3000); // Desaparece después de 6 segundos
    }
  }, [visible]);

  useEffect(() => {
    if (!show) {
      setTimeout(onClose, 800); // Espera a que termine la transición antes de ocultarlo completamente
    }
  }, [show, onClose]);

  return (
    <div
      className={`fixed top-5 left-5 z-50 bg-green-500 text-white shadow-lg rounded-lg px-5 py-4 border-l-4 border-green-700 flex items-start max-w-md ${
        show ? "slide-in" : "slide-out"
      }`}
    >
      <div className="bg-green mr-5 flex h-[28px] w-full max-w-[40px] items-center justify-center rounded-md">
        <svg
          width="18"
          height="18"
          viewBox="0 0 18 18"
      ...
      <div className="w-full">
        <h5 className="mb-3 text-lg font-bold text-[#004434]">{encabezado}</h5>
        <p className="text-base leading-relaxed text-body-color">{message}</p>
      </div>
    </div>
  );
};
